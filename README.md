...
Everything lands in `$(brew --prefix)/var/ai-proxy-service` — `config.yaml`, `.env`, `secrets.env`, `data/`. Point `database.dsn` or `DATABASE_URL` at a reachable PostgreSQL (or MySQL 8.0.13+), then run it as a service or by hand:

```sh
brew services start ai-proxy-service
# or: ai-proxy-service serve --config "$(brew --prefix)/var/ai-proxy-service/config.yaml"
```

`brew upgrade ai-proxy-service` upgrades to the latest release — the tap's formula is regenerated by the release workflow, and startup runs migrations. The in-app self-update is disabled in the seeded config; brew owns the binary.
